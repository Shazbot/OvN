...
------------------slave thing
araby_raid_slaves = {
	culture_key = "ovn_araby",
	pooled_resource_key = "def_slaves",
	pooled_resource_factor_key = "raiding",
    exclude_faction_key="ovn_arb_aswad_scythans"
}

function araby_raid_slaves:start_listeners()
	...
	local human_factions = cm:get_human_factions()
		...
		if current_human_faction:culture() == self.culture_key and current_human_faction:name() ~= self.exclude_faction_key then
	...
	core:add_listener(
		"rhox_araby_raiding_calculate_slaves",
		"ForceAdoptsStance",
		function(context)
			local faction = context:military_force():faction()
			
			return faction:is_human() and faction:culture() == self.culture_key and faction:name()~= self.exclude_faction_key
		end,
		function(context)
			local faction = context:military_force():faction()
	...
	core:add_listener(
		"rhox_araby_raiding_calculate_slaves_disband",
		"UnitDisbanded",
		function(context)
	...
	core:add_listener(
		"rhox_araby_raiding_calculate_slaves_post_movement",
		"CharacterFinishedMovingEvent",
		function(context)
			local character = context:character()
			local faction = character:faction()
			
			return faction:is_human() and faction:culture() == self.culture_key and cm:char_is_general_with_army(character) and character:military_force():active_stance() == "MILITARY_FORCE_ACTIVE_STANCE_TYPE_LAND_RAID" and faction:name()~= self.exclude_faction_key
		end,
		function(context)
	...
	core:add_listener(
		"rhox_araby_raiding_add_slaves",
		"FactionTurnStart",
		function(context)	
            local faction = context:faction()
			return context:faction():culture() == self.culture_key and faction:name()~= self.exclude_faction_key
		end,
		function(context)
			local faction = context:faction()
...
function araby_raid_slaves:calculate_slaves(force)
	local value = 0
...
function araby_raid_slaves:update_all_raiding_armies(faction_interface)
...
function araby_raid_slaves:get_army_count_raiding_region(region_interface, faction_interface)
	local army_count = 0
...
cm:add_first_tick_callback(function() 
    araby_raid_slaves:start_listeners()
end)