...
local function ovn_araby_determine_siege_equipment_bundle(cqi, unit_key) --yeah vanilla function will still spit out script errors
	if string.find(unit_key, "_arb_") then
		cm:apply_effect_bundle_to_force("wh_main_bundle_force_siege_equipment_arb", cqi, 0);
	end;
end;
    ...
    function(context)
        ...
        for i = 0, char_list:num_items() - 1 do
            ...
            if current_char:has_military_force() then
                ...
                ovn_araby_determine_siege_equipment_bundle(cqi, unit_key)
            end;
        end;
    end,
    true
