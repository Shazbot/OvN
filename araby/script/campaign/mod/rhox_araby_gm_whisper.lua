...
local tzarkan_mission_rewards = {
	...
	{
		key = "slaves",
		payload = "faction_pooled_resource_transaction{resource def_slaves;factor missions;amount ",
		weight = 2,
		min_amount = 500,
		max_amount = 2000,
		roundTo = 50
	...
	{
		key = "infamy",
		payload = "faction_pooled_resource_transaction{resource cst_infamy;factor missions;amount ",
		weight = 2,
		min_amount = 100,
		max_amount = 500,
		roundTo = 5
	...
	{
		key = "unit",
		--"add_mercenary_to_faction_pool{unit_key "..unit_choice..";amount "..unit_count..";}"
		payload = "add_mercenary_to_faction_pool{unit_key ",
		weight = 2,
	},
	{
		key = "ancillary",
...
local rhox_araby_tze_items ={
    ...
    "wh3_main_anc_follower_tze_weaver",
...
local rhox_araby_tze_units={
    {
        unit = "ovn_arb_tze_inf_blue_horrors_0",
        count = 3
    },
    {
        unit = "ovn_arb_tze_inf_pink_horrors_0",
        count = 2
    },
    {
        unit = "ovn_arb_tze_mon_screamers_0",
        count = 3
    },
    {
        unit = "ovn_arb_tze_mon_flamers_0",
        count = 2
    }
}
...
local function rhox_araby_get_unit_detail()
    local index = cm:random_number(#rhox_araby_tze_units,1)
    
    return rhox_araby_tze_units[index].unit..";amount "..rhox_araby_tze_units[index].count..";}"
end
...
local function rhox_araby_gm_tzarkan_generate_reward(harshness)
	...
	if reward.key == "ancillary" then
		local anc_key = get_random_ancillary_key_for_faction("ovn_arb_golden_fleet", false, "rare");
		if cm:model():random_percent(50) then
            anc_key = rhox_araby_tze_items[cm:random_number(#rhox_araby_tze_items, 1)]
		end
		payload = reward.payload..anc_key..";}";
    elseif reward.key == "infamy" then
		local amount = reward.min_amount + ((reward.max_amount - reward.min_amount) * harshness);
		payload = reward.payload..tostring(math.ceilTo(amount, reward.roundTo))..";context absolute;}";
     elseif reward.key == "unit" then
		local unit_detail = rhox_araby_get_unit_detail()
		payload = reward.payload..unit_detail
    elseif reward.key == "slaves" then
		local amount = reward.min_amount + ((reward.max_amount - reward.min_amount) * harshness);
		payload = reward.payload..tostring(math.ceilTo(amount, reward.roundTo))..";context absolute;}";
	else
		local amount = reward.min_amount + ((reward.max_amount - reward.min_amount) * harshness);
...
local function rhox_araby_gm_get_regions_within_distance_of_character(character, distance, not_razed, return_unique_list)
...
local function rhox_araby_gm_get_border_regions_of_faction(faction, outside_border, return_unique_list)
	-- These two functions no longer exist in wh3 has this was the only place it was used. Copying them here locally to make the script work but if they are needed in future we can look at making them baseline again
...
local function rhox_araby_gm_tzarkan_faction_war_weight(tzarkan_faction, faction_key)
...
local function rhox_araby_gm_tzarkan_generate_war_mission(tzarkan_faction)
	...
	for i = 1, #possible_factions_table do
		...
		if exclude == false then
			local faction_obj = cm:model():world():faction_by_key(faction_key);
			local weight = rhox_araby_gm_tzarkan_faction_war_weight(tzarkan_faction, faction_key);
	...
	if #weighted_factions.items > 0 then
		...
		local mm = mission_manager:new(tzarkan_faction_key, "rhox_araby_gm_tzarkan_declare_war");
        --local mm = mission_manager:new(tzarkan_faction_key, "rhox_araby_gm_tzarkan_declare_war");
		mm:set_mission_issuer("CLAN_ELDERS");
		mm:add_new_objective("DECLARE_WAR");
		mm:add_condition("faction "..selected_faction.faction_key);
        --mm:add_heading("missions_text_text_rhox_araby_gm_tzarkan_declare_war_title")
        --mm:add_description("mission_text_text_rhox_araby_gm_tzarkan_declare_war")
        --mm:add_condition("override_text mission_text_text_rhox_araby_gm_tzarkan_declare_war")
		if tazrkan_war_mission_turn_limit > 0 then
			mm:set_turn_limit(tazrkan_war_mission_turn_limit);
		end
		mm:set_should_whitelist(false);

		local reward = rhox_araby_gm_tzarkan_generate_reward(selected_faction.normalized_weight);
...
local function rhox_araby_gm_tzarkan_generate_raze_mission(tzarkan_faction)
	...
	if tazrkan_raze_mission_max_distance > 0 then
		region_list = rhox_araby_gm_get_regions_within_distance_of_character(faction_leader, tazrkan_raze_mission_max_distance, true, false);
	else
		region_list = rhox_araby_gm_get_border_regions_of_faction(tzarkan_faction, true, false);
	end
    out("Rhox araby gm number of regions: "..#region_list)
	...
	for i = 1, #region_list do
        ...
        if region_owner_key == tzarkan_faction_key then
            skip_faction = true;
        end
		...
		if skip_faction == false then
			...
			local war_harshness = rhox_araby_gm_tzarkan_faction_war_weight(tzarkan_faction, region_owner_key);
	...
	if #weighted_regions.items > 0 then
		...
		if #target_regions > 0 then
            local mm = mission_manager:new(tzarkan_faction_key, "rhox_araby_gm_tzarkan_raze_or_sack");
            --local mm = mission_manager:new(tzarkan_faction_key, "rhox_araby_gm_tzarkan_raze_or_sack");
			mm:set_mission_issuer("CLAN_ELDERS");
            ...
            --mm:add_heading("missions_text_text_rhox_araby_gm_tzarkan_raze_or_sack_title")
            --mm:add_description("mission_text_text_rhox_araby_gm_tzarkan_raze_or_sack")
            --mm:add_condition("override_text mission_text_text_rhox_araby_gm_tzarkan_raze_or_sack")
			local turn_limit = 0;
			...
			local reward = rhox_araby_gm_tzarkan_generate_reward(total_normalized_weight);
			mm:add_payload(reward);
			--mm:add_payload("faction_pooled_resource_transaction{resource def_malus_sanity;factor wh2_dlc14_resource_factor_sanity_tzarkans_whispers;amount "..tzarkan_possession_reward..";context absolute;}");
			mm:trigger();
...
local function rhox_araby_gm_tzarkan_generate_kill_mission(tzarkan_faction)
	out.design("tzarkan_generate_kill_mission()");
	local tzarkan_faction_key = tzarkan_faction:name();
	out.design("\tFaction: "..tzarkan_faction_key);
    
    local mm = mission_manager:new(tzarkan_faction_key, "rhox_araby_gm_tzarkan_kill_entities");
	--local mm = mission_manager:new(tzarkan_faction_key, "rhox_araby_gm_tzarkan_kill_entities");
	mm:set_mission_issuer("CLAN_ELDERS");
    ...
    --mm:add_heading("missions_text_text_rhox_araby_gm_tzarkan_kill_title")
    --mm:add_description("mission_text_text_rhox_araby_gm_tzarkan_kill_entities")
    --mm:add_condition("override_text mission_text_text_rhox_araby_gm_tzarkan_kill_entities")
	if tazrkan_kill_mission_turn_limit > 0 then
		mm:set_turn_limit(tazrkan_kill_mission_turn_limit);
	end
	mm:set_should_whitelist(false);
	
	local reward = rhox_araby_gm_tzarkan_generate_reward(normalized_kills);
	mm:add_payload(reward);
	--mm:add_payload("faction_pooled_resource_transaction{resource def_malus_sanity;factor wh2_dlc14_resource_factor_sanity_tzarkans_whispers;amount "..tzarkan_possession_reward..";context absolute;}");
	mm:trigger();
...
local function rhox_araby_gm_tzarkan_generate_mission(faction)
	if faction == nil then
	...
	while mission_generated == false and #mission_types.items > 0 do
		...
		if mission ~= nil then
			if mission == "DECLARE_WAR" then
				mission_generated = rhox_araby_gm_tzarkan_generate_war_mission(faction);
			elseif mission == "RAZE_OR_SACK" then
				mission_generated = rhox_araby_gm_tzarkan_generate_raze_mission(faction);
			elseif mission == "KILL_X_ENTITIES" then
				mission_generated = rhox_araby_gm_tzarkan_generate_kill_mission(faction);
			end
...
local function rhox_araby_gm_tzarkan_attempt_trigger_mission(faction)
	--out.design("tzarkan_attempt_trigger_mission()");
	local turn_number = cm:model():turn_number();
	
    out("Rhox araby gm Turn number: "..turn_number)
	if turn_number == tzarkan_generate_start_mission_turn then
		if tzarkan_generate_start_mission_count == -1 then
			...
			local mission_generated_1 = rhox_araby_gm_tzarkan_generate_war_mission(faction);
			out.design("\t\tWar Mission Generated: "..tostring(mission_generated_1));
			
			local mission_generated_2 = rhox_araby_gm_tzarkan_generate_raze_mission(faction);
			out.design("\t\tRaze Mission Generated: "..tostring(mission_generated_2));
			
			local mission_generated_3 = rhox_araby_gm_tzarkan_generate_kill_mission(faction);
		...
		elseif tzarkan_generate_start_mission_count > 0 then
			...
			for i = 1, tzarkan_generate_start_mission_count do
				rhox_araby_gm_tzarkan_generate_mission(faction);
			end
		end
	elseif turn_number > tzarkan_generate_start_mission_turn then
        out("Rhox araby gm cooldown: "..tzarkans_whispers.cooldown)
		if tzarkans_whispers.cooldown < 1 then
			...
			if cm:model():random_percent(chance_to_trigger) then
				out.design("\tSuccess!");
				local mission_generated = rhox_araby_gm_tzarkan_generate_mission(faction);
...
local function rhox_araby_gm_tzarkan_update_ui()
	local ui_root = core:get_ui_root();
	local mission_ui = find_uicomponent(core:get_ui_root(), "hud_campaign", "resources_bar_holder", "resources_bar", "rhox_araby_gm_malus_holder", "rhox_araby_gm_top_bar", "tzarkans_whispers");
	
	if mission_ui then
		mission_ui:InterfaceFunction("SetMissions", "rhox_araby_gm_tzarkan_declare_war", "rhox_araby_gm_tzarkan_kill_entities", "rhox_araby_gm_tzarkan_raze_or_sack");
    else
        out("Rhox GM Whisper: Could not found tzarkan_mission_types")
	end
end
...
local function rhox_araby_gm_add_tzarkans_whispers_listeners()

	out("#### Adding Tz'arkans Whispers Listeners ####");
	core:add_listener(
		"rhox_araby_gm_tzarkan_FactionTurnStart",
		"FactionTurnStart",
		function(context)
		...
		function(context)
			--local eye_frame = find_uicomponent(core:get_ui_root(), "hud_campaign", "resources_bar_holder", "resources_bar", "rhox_araby_gm_malus_holder", "rhox_araby_gm_top_bar", "tzarkans_whispers", "frame")
            --eye_frame:SetTooltipText(common.get_localised_string("campaign_localised_strings_string_rhox_araby_gm_whispers_inactive"), true) --might not work.
            rhox_araby_gm_tzarkan_attempt_trigger_mission(context:faction());
            rhox_araby_gm_tzarkan_update_ui();
		end,
		true
	);
	...
	--[[
    core:add_listener(
        "rhox_araby_gm_whisper_quest_panel_open_listener",
        "PanelOpenedCampaign",
        function(context)
			return context.string == "malus_quest_details";
		end,
        function()
            --out("Rhox araby gm: You here?")
            local quest_portrait = find_uicomponent(core:get_ui_root(), "hud_campaign", "resources_bar_holder", "resources_bar", "rhox_araby_gm_malus_holder", "rhox_araby_gm_top_bar", "tzarkans_whispers", "mission_list", "malus_quest_details", "quest_details_mask_holder", "quest_list_details_holder", "border_top", "malus_portrait")
            quest_portrait:SetImagePath("ui/skins/mod/rhox_araby_gm_empty_border.png", 0)
        end,
        true
    )
    --]]
...
cm:add_first_tick_callback(
    function()
        rhox_araby_gm_add_tzarkans_whispers_listeners()
        if cm:get_local_faction_name(true) == tzarkans_whispers.faction then--ui thing
            
            local parent_ui = find_uicomponent(core:get_ui_root(), "hud_campaign", "resources_bar_holder", "resources_bar");
            local result = core:get_or_create_component("rhox_araby_gm_malus_holder", "ui/campaign ui/rhox_araby_gm_malus_holder.twui.xml", parent_ui)
            if not result then
                script_error("Rhox araby gm Whisper: ".. "ERROR: could not create whisper ui component? How can this be?");
                return false;
            end;
            ...
            --[[
            local eye_frame = find_uicomponent(core:get_ui_root(), "hud_campaign", "resources_bar_holder", "resources_bar", "rhox_araby_gm_malus_holder", "rhox_araby_gm_top_bar", "tzarkans_whispers", "frame")
            eye_frame:SetTooltipText(common.get_localised_string("campaign_localised_strings_string_rhox_araby_gm_whispers_inactive"), true)
            --]]
        end
    end
);
...
cm:add_saving_game_callback(
	function(context)
		cm:save_named_value("rhox_araby_gm_tzarkans_whispers", tzarkans_whispers, context);
		cm:save_named_value("rhox_araby_gm_tzarkan_mission_types", tzarkan_mission_types, context);
	end
);
cm:add_loading_game_callback(
	function(context)
		if cm:is_new_game() == false then
			tzarkans_whispers = cm:load_named_value("rhox_araby_gm_tzarkans_whispers", tzarkans_whispers, context);
			tzarkan_mission_types = cm:load_named_value("rhox_araby_gm_tzarkan_mission_types", tzarkan_mission_types, context);
		end
	end
);