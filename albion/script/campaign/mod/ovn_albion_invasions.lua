OVN_ALBION_INVASION = OVN_ALBION_INVASION or {}
local mod = OVN_ALBION_INVASION

local ALBION_REGION_KEY =  "wh3_main_combi_region_konquata"
local COL_REGION_KEY =  "wh3_main_combi_region_citadel_of_lead"
local IOW_REGION_KEY =  "wh3_main_combi_region_isle_of_wights"
...
mod.invasion_queue = mod.invasion_queue or {}
...
local function albion_mist_invasion_queue(region)
	...
	table.insert(mod.invasion_queue, {
	...
	cm:add_interactable_campaign_marker("ovn_invasion_"..tostring(#mod.invasion_queue), "ovn_albion_invasion_marker_2", x, y, 1, albion_faction_key, "")
...
local function albion_mist_invasion_start(region, x, y)
    ...
    elseif random_number == 2 then
        faction = "wh2_dlc13_nor_norsca_invasion"
        army = random_army_manager:generate_force("ovn_albion_mist_norsca_force", upa, false);
        cm:change_custom_faction_name("wh2_dlc13_nor_norsca_invasion", "Be'lakor's Defilers")
    ...
    elseif region == COL_REGION_KEY then
        cm:show_message_event_located(
            albion_faction_key,
    ...
    elseif region == IOW_REGION_KEY then
        cm:show_message_event_located(
            albion_faction_key,
...
local function handle_invasion_queue()
	...
	for i=#mod.invasion_queue, 1, -1 do
		local invasion_data = mod.invasion_queue[i]
		local marker_id = "ovn_invasion_"..tostring(#mod.invasion_queue)

		if current_turn_number - invasion_data.start_turn >= 2 then
			table.remove(mod.invasion_queue, i)
		...
		elseif current_turn_number - invasion_data.start_turn >= 1 then
			cm:remove_interactable_campaign_marker(marker_id)
			cm:add_interactable_campaign_marker(marker_id, "ovn_albion_invasion_marker_1", invasion_data.x, invasion_data.y, 1, albion_faction_key, "")
		end
	end
end
...
core:add_listener(
	...
	function(context)
		...
		cm:remove_effect_bundle_from_faction_province("ovn_albion_waystone_build_time", context:building():region())
	end,
	true
)
...
cm:add_saving_game_callback(
	function(context)
		cm:save_named_value("ovn_albion_invasion_queue", mod.invasion_queue, context)
	end
)

cm:add_loading_game_callback(
	function(context)
		mod.invasion_queue = cm:load_named_value("ovn_albion_invasion_queue", mod.invasion_queue, context)
	end
)
