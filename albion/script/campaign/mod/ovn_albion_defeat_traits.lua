local ovn_albion_defeat_traits = {
	["bl_elo_dural_durak"] = {trait = "ovn_bl_elo_dural_durak_defeat_trait"},
	["albion_morrigan"] = {trait = "ovn_albion_morrigan_defeat_trait"}
}


local function ovn_albion_get_enemy_legendary_lords_in_last_battle(character)
	...
	for i = 1, num_attackers do
		...
		if char_obj:is_null_interface() == false then
			...
			if ovn_albion_defeat_traits[char_subtype] then
...
local function ovn_albion_add_defeated_trait_listeners()

    core:add_listener(
        "ovn_albion_defeat_traits_listener",
        "CharacterCompletedBattle",
        true,
        function(context)
            local character = context:character()
            if cm:char_is_general_with_army(character) and character:won_battle() then
                local enemy_LL = ovn_albion_get_enemy_legendary_lords_in_last_battle(character)
                out()
                for i = 1, #enemy_LL do
                    local LL_details = ovn_albion_defeat_traits[enemy_LL[i]]
                    
                    if LL_details ~= nil then
                        local trait = LL_details.trait	
                        out("OvN Albion defeat trait is: "..trait)
...
cm:add_first_tick_callback(function() ovn_albion_add_defeated_trait_listeners() end)
