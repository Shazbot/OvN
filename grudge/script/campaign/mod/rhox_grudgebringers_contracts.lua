...
RHOX_GRUDGEBRINGER_CONTRACT = {
    gb_faction= "ovn_emp_grudgebringers",
	mission_types = {
	...
	num_contracts = 3,
	turn_to_issue_contracts = 1,
	rhox_missing_ror_chance_contract = 20
}

cm:add_first_tick_callback(
	function()
		if cm:get_local_faction_name(true) == RHOX_GRUDGEBRINGER_CONTRACT.gb_faction then
            ...
            local result = core:get_or_create_component("rhox_button_contracts", "ui/campaign ui/rhox_grudgebringer_contract_button.twui.xml", parent_ui)
            ...
            result:SetContextObject(cco("CcoCampaignFaction", RHOX_GRUDGEBRINGER_CONTRACT.gb_faction))
            result:SetTooltipText(common.get_localised_string("uied_component_texts_localised_string_ComponentTooltip_84593183"), true)
            out("Rhox Test: "..common.get_localised_string("uied_component_texts_localised_string_ComponentTooltip_84593183"))
		end

        if cm:get_faction(RHOX_GRUDGEBRINGER_CONTRACT.gb_faction) and cm:get_faction(RHOX_GRUDGEBRINGER_CONTRACT.gb_faction):is_human() then
            RHOX_GRUDGEBRINGER_CONTRACT:setup_ogre_contracts()
        end
	end
)
...
function RHOX_GRUDGEBRINGER_CONTRACT:setup_ogre_contracts()
	
    local faction = cm:get_faction(self.gb_faction)
	if not faction or not faction:is_human() then
        return
    end
	
	common.set_context_value("rhox_grudgebringer_bounties_turn_start", self.turn_to_issue_contracts)
	...
	core:add_listener(
		...
		function(context)
			...
			return cm:model():turn_number() >= self.turn_to_issue_contracts and faction:is_human() and faction:name() == self.gb_faction
		end,
		function(context)
...
function RHOX_GRUDGEBRINGER_CONTRACT:attempt_to_issue_ogre_contracts(faction_key, num_contracts)
	...
	for i = 0, factions_met:num_items() - 1 do
		...
		if not faction:at_war_with(current_faction) and not current_faction:is_dead() and not current_faction:is_human() and current_faction:has_home_region() and RHOX_GRUDGEBRINGER_GOOD_CULTURE[current_faction:culture()] then --have to be a good culture and added condition
	...
	for i = 1, num_contracts do
		...
		if #filtered_enemies == 0 then
			...
			for j = 0, selected_issuing_faction:factions_met():num_items() - 1 do
				...
				if not current_faction_met:is_human() and not current_faction_met:is_dead() and not current_faction_met:has_effect_bundle("wh3_main_bundle_realm_factions") and not current_faction_met:has_effect_bundle("wh3_main_bundle_rift_factions") 
                and not selected_issuing_faction:is_vassal_of(current_faction_met) and not current_faction_met:is_vassal_of(selected_issuing_faction) and not RHOX_GRUDGEBRINGER_GOOD_CULTURE[current_faction_met:culture()] then --have to be a bad guy
		...
		if obj_str then
			local mod = 2.1 + (faction:bonus_values():scripted_value("contracts_treasury_payload_modifier", "value") / 100);
            
            local money = math.max(math.floor(selected_issuing_faction:treasury() / 5) + 500, 1650) * mod;
			...
			-- add bonus payloads
			local random_payload = cm:random_number(2)
